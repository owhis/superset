...
4. **Workspace MCP config** - keep shared MCP servers in `.mcp.json`; `.cursor/mcp.json` should link to `../.mcp.json`. Codex uses `.codex/config.toml` (run with `CODEX_HOME=.codex codex ...`). OpenCode uses `opencode.json` and Amp uses `.amp/settings.json`; both should mirror the same MCP set using their native schemas (`mcp` for OpenCode, `amp.mcpServers` for Amp).
