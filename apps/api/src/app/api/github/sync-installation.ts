...
interface SyncGithubInstallationParams {
...
export async function syncGithubInstallation({
	...
	console.log(`[${logPrefix}] Found ${repos.length} repositories`);

	const accessibleRepoIds = repos.map((repo) => String(repo.id));
		...
		await db
			...
			.where(
				and(
					...
					notInArray(githubRepositories.repoId, accessibleRepoIds),
				),
			);
	}
	...
	for (const repo of repos) {
		...
		console.log(
			`[${logPrefix}] Found ${prs.length} PRs (last 30 days) for ${repo.full_name}`,
		);
	...
	console.log(`[${logPrefix}] Sync completed successfully`);
